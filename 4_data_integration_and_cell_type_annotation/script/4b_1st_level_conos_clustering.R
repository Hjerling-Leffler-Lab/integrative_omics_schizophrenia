#about: Clustering snRNAseq - Human Cortex SCZ - Conos, Fig. S2 A-C
#Author: José A. Martínez-López, restructured Lisa Bast
#date: Created on Dec 6, 2020 updated on Dec 7,2021
#version: 0.0.2
...
data_path <- paste0(main_path,"/output/Conos_clustering/metadata/")
results_path <- paste0(main_path,'/output/Conos_clustering/1st_level/')
NCORES<-12

samples<-readRDS(file=paste0(data_path,"SamplesList_TH_and_D_adj_filtered_and_CT_annotated_20211217.rds"))
Disease<-readRDS(file=paste0(data_path,"Disease.rds"))
Donor<-readRDS(file=paste0(data_path,"Donor.rds"))
Age<-readRDS(file=paste0(data_path,"Age.rds"))
scmap_ann<-readRDS(file=paste0(data_path,"scmap_ann_76CT.rds"))
scmap_score<-readRDS(file=paste0(data_path,"scmap_score_76CT.rds"))
...
#Global clusters
con$findCommunities(method=leiden.community, resolution=7)
