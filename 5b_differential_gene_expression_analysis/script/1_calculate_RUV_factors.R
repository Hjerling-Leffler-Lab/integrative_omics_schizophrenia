#about: calculate RUV factors for specific model
#       prior to DESeq2 run
#author: Lisa Bast
#date: 15.02.22
#version: 0.0.1
...
opt_load_metadata <- TRUE#FALSE#  otherwise missing values are imputed 
...
#RUV settings:
n_RUV_factors_range <- c(6) #c(4,6,8)
...
filename_data_sample_info <- "T1_basic_donor_information.xlsx" #"sample_info_SUN_SCZ.xlsx"
filename_QC_data <- "T2_quality_metrics_per_sample.xlsx"  #"metrics_summary_tidy.csv" 
...
if (opt_downsampled_data ==TRUE){
  subfolder <- "Downsampled_data/"
  i <- commandArgs(trailingOnly=TRUE) # which downsampled file (0-9)
} else{
  subfolder <- ""
  i <- NaN
}
...
#path_expr_data <- paste0(main_path,'/output/CT_clustered_aggregated_data/',subfolder)
#path_QC_data <- paste0(main_project_path,'/3_quality_control/output/')
#path_sample_info_data <- paste0(main_project_path,'/2_alignment/output/')
#path_xy_chr_list <- paste0(main_path,'/data/')
#path_CT_spec_file<-paste0(main_path,'/data/speMx_Expr_',n_cluster,'CTs.Rdata')

#path_results_RUV <- paste0(main_path,'/output/RUV/')


## 0) load data:
# a) expression data (aggregated over all cells from a specific sample for each gene): 
data_all <- load_count_data_aggregated_across_celltypes(paths, code_path, opt_celltype_groups, opt_aggregation, add_str_pagoda, opt_downsampled_data, i)
...
for (n_RUV_factors in n_RUV_factors_range){
  #create and get results subfolder:
  paths$res_RUV <- get_results_folder(paths$results_RUV,paste0('design_with_',as.character(n_RUV_factors),'RUV_covariates_',opt_aggregation), data_all$results_subfolder,i,FALSE)
  ...
  plot_RUV_factor_correlation_with_metadata(RUV_factors,data_all$metadata,paths$res_RUV)
}
