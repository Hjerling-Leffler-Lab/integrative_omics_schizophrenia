...
#         Fig. 2 B-E,H, S6 A-E
#         Table T7: T7_DEGs_per_cell_type.csv
...
DESeq2_design_mode = 'with_6_RUVs' # 'without_covariates'
DESeq2_aggregation_method ='sum'
filename_DEG_results = 'df_results_no_shrinkage_all_genes.csv'
#if DESeq2_design_mode=='without_covariates':
#    filename_DEG_results = 'df_results_no_shrinkage_all_genes.csv'
#elif DESeq2_design_mode=='with_6_RUVs':
#    filename_DEG_results = 'df_results_'+DESeq2_aggregation_method+'_RUV.csv'
...
if genelist_version=="v3":
    gene_matrix_file = path_project + "5b_differential_gene_expression_analysis/data/geneMatrix_v2.tsv"
    gene_sets_file = path_project + "5b_differential_gene_expression_analysis/data/gsa-files-"+genelist_version+"/genesetsB.tsv"
else:
    gene_matrix_file = path_project + "5b_differential_gene_expression_analysis/data/geneMatrix_"+genelist_version+".tsv"
    gene_sets_file = path_project + "5b_differential_gene_expression_analysis/data/gsa-files-"+genelist_version+"/genesets.tsv"
bool_first_alpha = True
for alpha_val in alpha_val_range:

    # read data as anndata object
    #investigate results
    path_aggr_data, path_results_short = ut.get_paths(path_project,'DEG_visualization',"cell types")
    #create list of cell types:
    os.chdir(path_aggr_data)
    str_start = 'Agg_counts_'+add_str_pagoda+'_TH_and_D_adj_filtered_'
    csv_aggr_files = [ f for f in os.listdir( os.curdir ) if os.path.isfile(f) and f.startswith(str_start) and f.endswith('_'+DESeq2_aggregation_method+'.csv')]
    CTs_assigned = [f.split("_filtered_")[1].split('_'+DESeq2_aggregation_method+'.csv')[0] for f in csv_aggr_files]
    CTs_assigned = np.unique(CTs_assigned)

    genes_excitatory=[]
    path_results_long = path_results_short + 'design_'+DESeq2_design_mode+'/'
    os.chdir(path_results_long)
    if opt_load_DEG_results:
        df_DESeq2_all_genes_all_CTs = pd.read_csv(path_results_long+'T7_DEGs_per_cell_type.csv')
    else:
        ...
        method_str = DESeq2_design_mode+'_'+DESeq2_aggregation_method
        for k,ctf in enumerate(celltype_folders):
            if "figures" in ctf:
                ...
                if os.path.isfile(filename_DEG_results):
                    df_DESeq2_all_genes = pd.read_csv(filename_DEG_results)
                    df_DESeq2_all_genes.rename(columns={"Unnamed: 0":"Gene"},inplace=True)
                    cols_keep=["Gene","baseMean","log2FoldChange","lfcSE","stat","pvalue","padj"]
                    df_DESeq2_all_genes=df_DESeq2_all_genes[cols_keep]
                ...
                if opt_analyze_correlation_with_genelists or opt_export_genes_ensemble or opt_comparison_with_genelists or opt_plot_log2FC_per_CT:
                    if bool_first_CT:
                        ...
                        if opt_analyze_correlation_with_genelists:
                            df_corr_genelists_up,df_corr_genelists_down,bool_first_CT = ut.get_comparison_DEG_pvals_to_genelists(df_DESeq2_all_genes_CT_i,opt_save,path_results_long,CT_i,alpha_val,cutoff_q_p,bool_first_CT,[],[],gene_matrix_file,genelist_version)
                        bool_first_CT=False
                    else:
                        ...
                        if opt_analyze_correlation_with_genelists:
                            df_corr_genelists_up,df_corr_genelists_down,bool_first_CT = ut.get_comparison_DEG_pvals_to_genelists(df_DESeq2_all_genes_CT_i,opt_save,path_results_long,CT_i,alpha_val,cutoff_q_p,bool_first_CT,df_corr_genelists_up,df_corr_genelists_down,gene_matrix_file,genelist_version)

                #make sure to be back to correct directory:
                os.chdir(ctf)
                
                if bool_first_alpha:
                    #df stores p-values and if log2FC was <0 or >0:
                    ...
                    DF_p_vals = pd.read_csv(path_results_long+'df_p_vals_adj_'+DESeq2_aggregation_method+'_per_celltype.csv')
                
                #get results for significant genes:
                #if os.path.isfile('df_results_'+DESeq2_aggregation_method+'.csv'):
                #    df_DESeq2 = pd.read_csv('df_results_'+DESeq2_aggregation_method+'.csv')
                #elif os.path.isfile('df_results_'+DESeq2_aggregation_method+'_RUV.csv'):
                #    df_DESeq2 = pd.read_csv('df_results_'+DESeq2_aggregation_method+'_RUV.csv')
                #elif os.path.isfile('df_results_'+DESeq2_aggregation_method+'_MARS.csv'):
                #    df_DESeq2 = pd.read_csv('df_results_'+DESeq2_aggregation_method+'_MARS.csv')
                #else:
                #    continue
                df_DESeq2 = pd.read_csv(filename_DEG_results)
                df_DESeq2 = ut.add_gene_names(df_DESeq2)
                
                # export all excitatory genes:
                if "Excitatory" in ctf:
                    genes_excitatory.append(df_DESeq2['genes'].tolist())
        ...
        # last CT: save some DFs
        df_DESeq2_all_genes_all_CTs.to_csv(path_results_long+'T7_DEGs_per_cell_type.csv')
        if opt_analyze_correlation_with_genelists:
        ...
        if bool_first_alpha:
            DF_p_vals.to_csv(path_results_long+'df_p_vals_adj_'+DESeq2_aggregation_method+'_per_celltype.csv')
            bool_first_alpha = False

    if alpha_val==0.05:
        #effect size per CT:
        if opt_plot_log2FC_per_CT:
            ut.plot_log2FC_per_CT(df_DESeq2_all_genes_all_CTs,alpha_val,16,path_filtered_loom_data,path_results_long,opt_save)

    #plot signed p-value pairwise correlation between cell types as clustermap
    if opt_plot_correlation_clustermap and alpha_val==alpha_val_range[0]:
        ut.plot_signed_DEG_corr_clustermaps(df_DESeq2_all_genes_all_CTs,path_results_long,alpha_val_range,opt_sign_in_one_ct)

    
    #number deregulated genes per cell type:
    if opt_plot_number_deregulated_genes:
        #something gets mixed up here --> continue fixing!!
        ut.plot_number_of_deregulated_genes(df_DESeq2_all_genes_all_CTs,'no_shrinkage',opt_save,path_results_long,'vs_celltypes',16,path_results_long,path_filtered_loom_data,add_str,add_str_pagoda,opt_celltype_groups,alpha_val,path_filtered_loom_data,gene_matrix_file,False)
        ut.plot_number_of_deregulated_genes(df_DESeq2_all_genes_all_CTs,'no_shrinkage',opt_save,path_results_long,'vs_number_cells',16,path_results_long,path_filtered_loom_data,add_str,add_str_pagoda,opt_celltype_groups,alpha_val,path_filtered_loom_data,gene_matrix_file,False)
    ...
    if alpha_val==0.05:
        ...
        if opt_comparison_with_genelists:
            genelists_wp, genelist_names_wp = ut.get_gene_lists(gene_matrix_file,genelist_version,gene_sets_file)
            if genelist_version == 'v1' or genelist_version == 'v2':
                for i,gl in enumerate(genelists_wp):
                    ut.plot_venn_geneMatrix(DF_DESeq2,gl,genelist_names_wp[i],opt_save,path_results_long,DESeq2_aggregation_method,'no_shrinkage','',alpha_val)
    
            #genelists plots (independent of alpha_val --> only run once)
            genelists, genelist_names, q_values = ut.get_gene_lists_with_qvalues(gene_matrix_file,genelist_version)
            ...
            if opt_comparison_with_genelists:
                ...
                if genelist_version == 'v1' or genelist_version == 'v2':
                    ut.plot_percentage_genes_in_list_and_DEG_for_range_of_cutoffs(gene_matrix_file, df_DESeq2_all_genes_all_CTs, opt_par, qval_cutoff_range, path_results_long, opt_save)
    ...
    #plot frequency of genes that are detected multiple times:
    ut.plot_DEGs_CT_overlap_as_upset(df_DESeq2_all_genes_all_CTs,opt_save,path_results_long,DESeq2_aggregation_method, '',alpha_val)
