...
about: visualize pathways found based on DEGs (number of genes in pathways and p-values of pathways, comparison with proteomics result)
       ...
       Table T5: T5_sign_pathways_per_cell_type.xlsx
...
path_info["subfolders"] = ['alpha05','alpha1','alpha3']
path_info["min_overlap_folders"] = ['min_overlap_3']
...
for sf in path_info["subfolders"]:
    if sf.split('alpha')[1] =="05":
        alpha_val_DEGs = int(sf.split('alpha')[1])/100
    else:
        alpha_val_DEGs = int(sf.split('alpha')[1])/10
    for ssf in path_info["min_overlap_folders"]:
        for i,dm in enumerate(DESeq2_design_modes):
            path_info["folder_p"] = path_info["path_results"]+"DEGs/"+options["opt_GSA_version"]+'/'+sf+'/'+ssf+'/'+ dm+'/'
            ...
            for j,bg in enumerate(background_data_transcriptomics):
                ...
                for directory in folders:
                    ...
                    if options["opt_plot_pathway_statistics"] or options["opt_create_DF_sorted_filtered"] or options["opt_plot_n_pathways_heatmap"]==True or options["opt_plot_pathway_statistics"]==True or options["opt_plot_PCA_pathways"]==False or options["opt_plot_upsets"]==True:
                        DF_sorted_filtered = ut.generate_GSA_output_and_statistics(path_info, dm, options, variable_gene_names, 15,alpha_val_DEGs,variables_groups,variable_n_genes,background_data_proteomics,directory,j,bg_str,results_path_proteomics)
                    else:
                    ...
                    ut.plot_triangular_heatmaps(DF_sorted_filtered,path_info, options,15,bg_str)
                        
    #export input data frame for GSA
    #ut.get_sign_Genes_with_sign_pathways_per_CT(DF_sign,path_results_long, 16, DESeq2_design_mode,alpha_val, True)