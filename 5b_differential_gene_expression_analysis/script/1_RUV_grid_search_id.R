#about: Build DESeq2 model by using different number of RUV variables
#       to decide how many RUV variables enter the model
...
if (n_RUV_factors_max<=10){
  RUV_factors_sequence <- seq(0,n_RUV_factors_max)
  print(RUV_factors_sequence)
} else{
  RUV_factors_sequence <- c(seq(0,10),seq(15,n_RUV_factors_max,by=5))
}
...
if (opt_run_RUV == TRUE & opt_run_split_halves==TRUE){
  ...
  #remove genes with low counts and on x/y chromosomes and store data in DDS objects:
