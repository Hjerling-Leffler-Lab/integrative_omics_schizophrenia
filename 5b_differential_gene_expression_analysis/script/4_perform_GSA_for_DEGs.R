...
alpha_DEGs_DAPs_range <- c(0.05,0.1,0.3)#this is the best resolution #c(0.05,0.1,0.3,0.5) # for selecting DEGS
alpha_pathways <- 0.05
...
for (opt_filter_for_protein_coding_genes in options_filter_for_protein_coding_genes){
  for (opt_background_data in opt_background_data_sets){
    ...
    for (alpha_val in alpha_DEGs_DAPs_range){
      #filename definition:
      if (opt_data_i == 'transcriptomics'){
        #how is this file created?? couldn't find the code yet
        ...
        for (min_number_genes_in_gene_set in number_genes_in_gene_set_range){
          for (n_RUV_vars_to_inlcude in n_RUV_vars_to_inlcude_range){
            if (n_RUV_vars_to_inlcude<=8){ # otherwise overfitting
              ...
              path_DEG_results <- paste0(main_path,"/output/DEGs/",add_str_design,"/")
              path_GSA_results <- paste0(main_path,"/output/GSA_analysis/",add_folder_str,version_nr,"/alpha",sub("\\.", "", as.character(alpha_val*10)),"/min_overlap_",as.character(min_number_genes_in_gene_set),"/",add_str_design_short,add_str_background,"/")
