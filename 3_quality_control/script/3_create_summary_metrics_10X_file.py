...
#@author: Lisa Bast
#extract QC metrics from several sources (10X summary metrics, filtering result, stats based on raw cound data) and summarize them in a dataframe/ .csv file
...
os.chdir(path_data)
df = pd.read_csv('metrics_summary_concatenated.csv')

# add IDs to df
df_IDs = pd.read_excel(path_data+"T1_basic_donor_information.xlsx", "basic_donor_information", engine='openpyxl') 
df_IDs.rename(columns = {'donor_ID_internal_7':'donor_ID_library'},inplace=True)
df_IDs.rename(columns = {'donor_ID_internal_8':'donor_ID_python'},inplace=True)
...
df_IDs['Library'] = df_IDs['donor_ID_library'].str[:4]
df_metrics = pd.merge(df_IDs[['donor_ID_library', 'donor_ID_universal','donor_ID_python','Group','Library']],df,on='donor_ID_library')
...
if opt_integrate_filtering_results:
    ...
    df_filt_result_pagoda.rename(columns = {'Donor ID':'donor_ID_python','Percentage Cells discarded':'p_cells_del_filt_pagoda'},inplace=True)
    ...
    df_filt_result_TH.rename(columns = {'Donor ID':'donor_ID_python','Percentage Cells discarded':'p_cells_del_filt_TH'},inplace=True)
    df_filt_result_D = pd.read_pickle(path_output+"filtered/df_pagoda_TH_and_D_adj_filtering_result_cellranger")
    df_filt_result_D.rename(columns = {'Donor ID':'donor_ID_python','Percentage Cells discarded':'p_cells_del_filt_D'},inplace=True)
    df_filt = pd.merge(df_filt_result_pagoda,df_filt_result_TH,on=['donor_ID_python',"Disease"],how='left')
    df_filt = pd.merge(df_filt, df_filt_result_D,on=['donor_ID_python',"Disease"],how='left')
    ...
    df_metrics1 = pd.merge(df_metrics,df_filt[['p_cells_del_filt','donor_ID_python']],on='donor_ID_python',how='left')
...
df_patientData.rename(columns = {'donor_ID_internal_8':'donor_ID_python'},inplace=True)
df_metrics = pd.merge(df_metrics1,df_patientData[['gender','age','PMI_h','donor_ID_python']],on='donor_ID_python',how='left')
...
if opt_integrate_raw_data_stats:
    ...
    df_raw_stats.rename(columns = {'Mean counts per barcode':'mean_counts_per_barcode','Std counts per barcode':'std_counts_per_barcode','Median genes per cell':'median_gpc','Donor ID':'donor_ID_python'},inplace=True)
    df_metrics = pd.merge(df_metrics,df_raw_stats[['mean_counts_per_barcode','std_counts_per_barcode','median_gpc','donor_ID_python']],on='donor_ID_python',how='left')
...
if opt_add_neuron_percentage == True:
    ...
    for f in files_all:
        if "_downsampled_" not in f:
            ...
            if bool_first:
                df_neuron_percentage = pd.DataFrame(data = {'donor_ID_python': sample_IDs, 'n_cells': n_cells, 'n_neurons': n_neurons})
                bool_first=False
            else:
                for s in sample_IDs:
                    df_neuron_percentage['n_cells'][df_neuron_percentage['donor_ID_python']==s] = df_neuron_percentage['n_cells'][df_neuron_percentage['donor_ID_python']==s].copy()+n_cells[sample_IDs==s][0]
                    df_neuron_percentage['n_neurons'][df_neuron_percentage['donor_ID_python']==s] = df_neuron_percentage['n_neurons'][df_neuron_percentage['donor_ID_python']==s].copy()+n_neurons[sample_IDs==s][0]
    df_neuron_percentage['p_neurons'] = (df_neuron_percentage['n_neurons']/df_neuron_percentage['n_cells'])*100
    df_metrics_new = pd.merge(df_metrics,df_neuron_percentage[['p_neurons','donor_ID_python']],on='donor_ID_python',how='left')
    
    #remove row if donor_ID_python is nan
    df_metrics_new = df_metrics_new.dropna(subset=['donor_ID_python'])
    df_metrics_new.to_excel(path_output+'T2_quality_metrics_per_sample.xlsx',index=False)
else:    
    #remove row if donor_ID_python is nan
