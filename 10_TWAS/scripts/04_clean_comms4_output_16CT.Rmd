...
lvl="15CT"
WD0=paste("./10_TWAS/data/TWAS_output/",lvl,sep="")

CT_list <- list.files(WD0)
...
for(i in 1:length(CT_list)){
  CT=CT_list[i]
  WD1=paste("./10_TWAS/data/",lvl,"/", CT, sep="")
  setwd(WD1)
  infiles <- list.files(pattern = "INPUT1_") %>% as.data.frame()
  colnames(infiles) <- "filename"
  input1 <- as.list(infiles$filename) %>%
    map_df(function(x) fread(x, stringsAsFactors=FALSE, data.table=FALSE))
  df.ngene[i,c(1,2)] <- c(lvl,CT)
  df.ngene[i,3] <- length(unique(input1$Gene))
}
df.ngene
rm(input1,i,CT,infiles,WD1)

#- clean output
for(CT in CT_list){
  if(CT%in%c("Astrocytes","Endothelial_and_mural_cells","Microglial_cells")){
    print(paste("skip cell type",CT, sep=" "))}else{
  WD=paste("./10_TWAS/data/TWAS_output/",lvl,"/",CT,sep="")
  setwd(WD)
  system("mkdir cleaned_output")

#-- per chr clean output
  ...
  for (i in 1:22){
    ...
    out=fm$out_param
    sum(fm$genetype1[out[,3]!=-99] ==
          fm$gene_type1)==nrow(fm$out_param0)
    ...
    lambda
    print(paste(CT,"chr",i,"lambda is:", lambda,sep=" "))
